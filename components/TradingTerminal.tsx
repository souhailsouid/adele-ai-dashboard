...
import { Activity, Users, Briefcase, Newspaper, Sparkles, AlertTriangle, Bell, X } from 'lucide-react'
...
export default function TradingTerminal({ alert, onClose }: TradingTerminalProps) {
  ...
  const getAlertTypeBadge = (alertItem: FlowAlert) => {
    if (alertItem.has_sweep) {
      return (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 bg-grid flex flex-col min-w-0">
          ...
          <div className="h-10 border-b border-white/5 flex items-center px-4 gap-4 bg-neutral-950/80 backdrop-blur shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setActiveChart(activeChart === 'greeks' ? null : 'greeks')}
              ...
              <button
                onClick={() => setActiveChart(activeChart === 'oi' ? null : 'oi')}
                className={`px-2 py-0.5 rounded text-[10px] transition-colors ${
                  activeChart === 'oi'
                    ? 'bg-orange-500/20 text-orange-400 border border-orange-500/20'
                    : 'hover:bg-white/5'
              ...
              <button
                onClick={() => setActiveChart(activeChart === 'regression' ? null : 'regression')}
                className={`px-2 py-0.5 rounded text-[10px] transition-colors ${
              ...
              <div className="w-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                  {ownership.slice(0, 5).map((holder, index) => (
                    <div
                      key={holder.cik || index}
                      className="bg-neutral-900/50 rounded-lg p-3 border border-white/5 hover:border-white/10 transition-colors cursor-pointer"
                      onClick={() => {
                        setSelectedInstitution(holder)
                          ...
                          <div className="grid grid-cols-2 gap-y-1 gap-x-4 mt-2">
                            <div>
                              ...
                              <div className="text-xs font-mono text-neutral-300">
                                {holder.ownership_perc !== undefined
                                  ? institutionalOwnershipService.formatOwnershipPercent(holder.ownership_perc)
                                  : 'N/A'}
                              </div>
                            </div>
                            <div className="text-right">
                              ...
                              <div className="text-xs font-mono text-neutral-300">
                                {holder.units?.toLocaleString() || holder.shares?.toLocaleString() || 'N/A'}
                              </div>
                            </div>
                            <div>
                              <div className="text-[10px] text-neutral-500">Market Value</div>
                              <div className="text-xs font-mono text-neutral-300">
                                {institutionalOwnershipService.formatMarketValue(holder.value)}
                              </div>
                            </div>
                            <div className="text-right">
